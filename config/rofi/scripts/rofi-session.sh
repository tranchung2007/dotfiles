#!/usr/bin/env bash
#
# this script manages the user session using systemctl and an optional screen locker
#
...
((ROFI_ICONS)) && rofi_flags="-show-icons -no-X"
...
suspend_sys() { $($USE_LOCKER) && "$LOCKER"; systemctl suspend; }
hibernate_sys() { $($USE_LOCKER) && "$LOCKER"; systemctl hibernate; }
